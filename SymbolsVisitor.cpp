...
antlrcpp::Any SymbolsVisitor::visitFunction(AslParser::FunctionContext *ctx) {
  ...
  if (Symbols.findInCurrentScope(ident)) {
    ...
    for (size_t i = 0; i < ctx->params().size(); ++i){ 
      lParamsTy.push_back(getTypeDecor(ctx->params(i)->type()));
    }
...
antlrcpp::Any SymbolsVisitor::visitType(AslParser::TypeContext *ctx) {
  ...
  TypesMgr::TypeId t;
  if (ctx->type_array()) {    
    visit(ctx->type_array());
    t = getTypeDecor(ctx->type_array());
  }
  else {   
    visit(ctx->basic_type());
...
antlrcpp::Any SymbolsVisitor::visitType_array(AslParser::Type_arrayContext *ctx) { 
  DEBUG_ENTER();
  unsigned int integer = std::stoi(ctx->INTVAL()->getText());

  visit(ctx->basic_type());

  TypesMgr::TypeId tipo = getTypeDecor(ctx->basic_type());
  
  TypesMgr::TypeId t = Types.createArrayTy(integer, tipo);
